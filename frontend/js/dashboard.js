...
// =====================
// OBTENER BALANCE TOTAL HISTORICO
// =====================
async function obtenerBalanceTotal() {
  try {
    const respuesta = await fetch(API_URL + "/balance-total", {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        "Authorization": "Bearer " + token
    ...
    el.textContent = formatearMoneda(datos.balance_total);

    if (datos.balance_total < 0) {
      el.style.color = "rgba(232, 107, 107, 0.9)";
    }
